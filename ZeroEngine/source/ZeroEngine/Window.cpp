...
namespace ZeroEngine::Window {
static bool s_GLFWInitilized = false;

static void glfwErrorCallback(int error, const char* message) {
	ZE_CORE_ERROR("GLFW Error: ({0}): {1}", error, message);
}
...
void Window::Init(const WindowData& data) {
	...
	ZE_CORE_INFO("Creating window {0} {1} {2}", data.Name, data.Width, data.Height);

	if (!s_GLFWInitilized) {
		glfwSetErrorCallback(glfwErrorCallback);

		if (!glfwInit()) {
			ZE_CORE_ERROR("Could not initialize GLFW!");
			return;
		}
...
void Window::SetVSync(bool enable) {
	if (m_Context == nullptr) {
		ZE_CORE_FATAL("To set vertical immobility, the context must be initialized (in Window.cpp)")
		exit(1);
	} else {
